            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                ...
                <form action="{{ route('booking.update', $booking->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="property_name" name="property_name" value="{{ $booking->property->name ?? 'Inconnue' }}" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md" readonly>
                    </div>
                    <div class="mb-4">
                        <label for="start_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date de début</label>
                        <input type="date" id="start-date" name="start_date" value="{{ $booking->start_date }}" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md" required>
                    </div>
                    <div class="mb-4">
                        <label for="end_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date de fin</label>
                        <input type="date" id="end-date" name="end_date" value="{{ $booking->end_date }}" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md" required>
                    </div>
                    <div class="mb-4">
                        <label for="total_price" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Prix total</label>
                        <input type="text" id="total_price" name="total_price" value="{{ number_format($booking->total_price ?? 0, 2) }} €" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md" readonly>
                    </div>
